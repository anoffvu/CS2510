...
abstract class ANode<T> {
  ...
  abstract void filterHelp(ANode<T> deque, IPred<T> pred);
...
class Sentinel<T> extends ANode<T> {
  ...
  @Override
  public void filterHelp(ANode<T> lastLink, IPred<T> pred) {
    lastLink.next = this;
    this.prev = lastLink;
...
class Node<T> extends ANode<T> {
  ...
  public void filterHelp(ANode<T> lastLink, IPred<T> pred) {
    if (pred.apply(this.data)) {
      lastLink.next = this;
      this.prev = lastLink;
      this.next.filterHelp(this, pred);
    }
    else {
      this.next.filterHelp(lastLink, pred);
    }
...
class ExamplesDeque {
  ...
  void testFilter(Tester t) {
    initData();
    this.deque2.filter(new SameString("abc"));
